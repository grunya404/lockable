(function (JGS, $, undefined) {
  ...
  JGS.ServerDataSimulator = function (seriesName, minDelay, maxDelay) {
    ...
    this.maxDelay = maxDelay === undefined ? 200 : maxDelay;
  ...
  JGS.ServerDataSimulator.prototype.loadData = function (dataLoadReq) {
    ...
    for (var i = 0; i < this.serverData.length; i++) {
      ...
      if (this.serverData[currIdx].time < (currTime - timePerInterval))
        currIdx++;
      else
        break;
    }
    ...
    while (currIdx < this.serverData.length && currTime < dataLoadReq.endDateTm.getTime()) {
      ...
      while (currIdx < this.serverData.length && this.serverData[currIdx].time < currTime) {
        sum += this.serverData[currIdx].value;
        min = Math.min(min, this.serverData[currIdx].value);
        max = Math.max(max, this.serverData[currIdx].value);
        currIdx++;
        numPoints++;
      }
  ...
  JGS.ServerDataSimulator.prototype._generateServerData = function () {
    ...
    if(true) {
      startMom = moment().utc().add(-2,'w');
      endMom = moment().utc().add(1,'d');
      if (this.seriesName == "Series-A") {
        majorInterval = moment.duration(11, 'hours');
        minorInterval = moment.duration(1, 'second');
      }
      else if (this.seriesName == "Series-B") {
        majorInterval = moment.duration(5, 'hours');
        minorInterval = moment.duration(5, 'second');
      }
      else {
        majorInterval = moment.duration(20, 'hours');
        minorInterval = moment.duration(10, 'second');
      }
    }
    else {     
      startMom = moment('2012-01-01').utc();
      endMom = moment('2015-01-01').utc();
      if (this.seriesName == "Series-A") {
